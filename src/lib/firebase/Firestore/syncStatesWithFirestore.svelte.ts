...
export const syncUserStateAndFirestore = function () {
    const unsubscribeUserState = userState.subscribe(() => {
        if (userState.get().lastUpdated > firebaseState.userDoc.lastUpdated) {
    ...
    const unsubscribeUserDoc = firebaseState.subscribeToUserDoc(() => {
        if (firebaseState.userDoc.lastUpdated === userState.get().lastUpdated) {
            console.log("User is synced.")
            return
        } else if (firebaseState.userDoc.lastUpdated > userState.get().lastUpdated) {
...
export const syncPagesStateAndFirestore = function () {
    const unsubscribeFromPagesState = pagesState.subscribe((id: string) => {
        if (!pagesState.get()[id] || !firebaseState.pageDocs[id] || pagesState.get()[id].lastUpdated > firebaseState.pageDocs[id].lastUpdated) {
            firebaseState.isPublishing = true;
    ...
    const unsubscribeFromFirebaseState = firebaseState.subscribeToPageDocs((id: string) => {
        if (!firebaseState.pageDocs[id]) {
            ...
            pagesState.get()[id] = firebaseState.pageDocs[id];
        }
        else if (firebaseState.pageDocs[id].lastUpdated === pagesState.get()[id].lastUpdated) {
